"""Verbatim copy of the pre-Stage-2 Denoiser, renamed LegacyDenoiser.

Used only by tests/test_multistream_pipeline.py to anchor the loss-parity
test. Production code must not import this module.
"""
import argparse
...
def _legacy_dual_b_2_4c_896(**kwargs):
...
_LEGACY_JIT_MODELS = {
    "JiT-Dual-B/2-4C-896": _legacy_dual_b_2_4c_896,
}


class LegacyDenoiser(nn.Module):
    def __init__(
        self,
        args: argparse.Namespace,
    ) -> None:
        super().__init__()
        self.net: nn.Module = _LEGACY_JIT_MODELS[args.model](
