...
def resolve_feature_dir_name(
    ...
    if default_name is None:
        raise ValueError("A feature directory name is required.")
    return maybe_append_split_suffix(default_name, split)
...
def feature_shard_dirs(dataset_root: str) -> List[str]:
    pattern = os.path.join(
        dataset_root,
        "shard_[0-9][0-9][0-9][0-9][0-9]_[0-9][0-9][0-9][0-9][0-9]",
    )
    return sorted(path for path in glob(pattern) if os.path.isdir(path))


def load_feature_shard_dataset(shard_dir: str, dataset_dir_name: str):
    state_path = os.path.join(shard_dir, "state.json")
    dataset_info_path = os.path.join(shard_dir, "dataset_info.json")
    file_states = {
        "state.json": describe_file_state(state_path),
        "dataset_info.json": describe_file_state(dataset_info_path),
    }
    missing_or_empty = [
...
def inspect_feature_shards(data_path: str, dataset_dir_name: str) -> FeatureShardStore:
    ...
    shard_dirs = feature_shard_dirs(dataset_root)
    if not shard_dirs:
        raise FileNotFoundError(f"No shard directories found under {dataset_root}")
    ...
    for shard_dir in shard_dirs:
        shard_dataset = load_feature_shard_dataset(shard_dir, dataset_dir_name)
...
def copy_rows_to_ram(rows: Dict[str, object], dtype: np.dtype) -> Dict[str, np.ndarray]:
    return {
...
def concat_rows(*row_groups: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
    return {
...
def load_feature_range_to_ram(
    ...
    while shard_idx < len(store.shard_spans):
        ...
        rows = (
            shard_dataset[:]
            if local_start == 0 and local_end == span.size
            else shard_dataset[local_start:local_end]
        )
        copied = copy_rows_to_ram(rows, dtype=dtype)
...
class PairedRamLoadedShardDataset(IterableDataset):
    ...
    def _select_logical_shard_store(self) -> FeatureShardStore:
        return max(
            (self.latent_store, self.dino_store),
            key=lambda store: (
    ...
    def _load_logical_shard(self, shard_span: LogicalShardSpan) -> Dict[str, np.ndarray]:
        latent_rows = load_feature_range_to_ram(
            self.latent_store,
            shard_span.global_start,
            shard_span.global_end,
        )
        dino_rows = load_feature_range_to_ram(
            self.dino_store,
            shard_span.global_start,
            shard_span.global_end,
        )
        ...
        return {
            ...
            "y": latent_rows["label"],
            "sample_id": latent_rows["sample_id"],
        }
