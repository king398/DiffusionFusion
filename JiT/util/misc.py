...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, header=None, steps_per_epoch:int=None):
        i = 0
        if not header:
            header = ''
...
def init_distributed_mode(args):
    ...
    dist_timeout_sec = int(getattr(args, "dist_timeout_sec", 7200))
...
def add_weight_decay(model, weight_decay=0, skip_list=()):
    ...
    for name, param in model.named_parameters():
        ...
        if len(param.shape) == 1 or name.endswith(".bias") or name in skip_list or 'diffloss' in name:
            # no weight decay on bias, norm and diffloss
